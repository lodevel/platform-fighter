/* THROWAWAY — verify donkeykong renders + per-move clips play in-engine. */
...
(async () => {
  ...
  await page.waitForFunction(
    () => window.__game && window.__game.textures && window.__game.textures.exists('char.donkeykong.neutral_special'),
    { timeout: 45000 },
  );
  await page.evaluate(() => {
    ...
    g.scene.start('MatchScene', { matchConfig: { mode: 'stocks', stockCount: 9, stageId: 'flat', rngSeed: 5, itemFrequency: 'off',
      players: [
        { index: 1, characterId: 'donkeykong', paletteIndex: 0, inputType: 'ai', aiDifficulty: 'hard' },
  ...
  const registered = await page.evaluate(() => {
    ...
    return moves.filter((m) => a.exists(`donkeykong.${m}.anim`));
  });
  const seen = {};
  for (let i = 0; i < 70; i++) {
    const k = await page.evaluate(() => {
      ...
      let key = null;
      const walk = (o) => { if (!o) return; if (o.anims && o.texture && String(o.texture.key).startsWith('char.donkeykong') && o.anims.currentAnim) key = o.anims.currentAnim.key; if (o.list) o.list.forEach(walk); };
  ...
  await page.screenshot({ path: 'assets/gen/_spike/donkeykong-ingame.png' });
